...
package com.levelup.minigame.common.managers.assets
{
	import br.com.stimuli.loading.BulkLoader;

	import com.greensock.events.LoaderEvent;
	import com.levelup.minigame.common.managers.assets.types.*;
	...
	{
		public static const mainLoader: BulkLoader = new BulkLoader('chp_lu_loader', 5, BulkLoader.LOG_SILENT);
		...
		public static function initialize (value: XMLList, callBack: Function): void
		{
			...
			{
				asset = AssetManager.createAsset(assetXML.@name, assetXML.@path, (assetXML.@checkLoad == "1"), false, assetXML.@type, callBack);
				if (asset.checkLoad)
				{
		...
		{
			GamePreloader.mainLoader.update();
		}
		...
		public static function isMustLoadAll():int
		{
			for each(var asset:IAsset in assets)
			{
				...
				if (asset.percent < 100)
                {
                    trace("loading ",asset.name,asset.percent,"%");
					return 1;
                }
