package com.api.forticom
{
	...
	public class ForticomAPI extends EventDispatcher
	{
		...
		public function tryToCreateConnection(event : TimerEvent = null) : void
		{
			...
			/**
			 * If client connection created successfully, but not proxy not yet connected to this client, we will start timer to monitor, that successful connection.
			 * 
			 */
			if (!this._connected) {
				if(event)
		...
		{
			if (DEBUG) trace ("Sending " + method + " (" + rest + ")");
			if (this._connected)
			{
				if (rest.length) {
		...
		{
			//ForticomAPI.instance.doSend("showPermissions", JSON.stringify(permissions));
